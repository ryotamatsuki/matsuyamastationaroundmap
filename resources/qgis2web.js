...
//initial view - fit to the extent of the polygon layer
map.getView().fit(jsonSource_polygon_GeoJSON_1.getExtent(), map.getSize());
...
function createPopupField(currentFeature, currentFeatureKeys, layer) {
    var popupText = '';
    const allowedFields = ['役割分担', '完成時期']; // Whitelist for popup fields
    for (var i = 0; i < currentFeatureKeys.length; i++) {
        const currentKey = currentFeatureKeys[i];
        // Only process allowed fields, and exclude internal OpenLayers keys
        if (allowedFields.includes(currentKey) &&
            currentKey !== 'geometry' && currentKey !== 'layerObject' && currentKey !== 'idO') {
            
            var popupField = '';
            // Existing logic to handle hidden/inline labels
            if (layer.get('fieldLabels')[currentKey] == "hidden field") {
                continue;
            } else if (layer.get('fieldLabels')[currentKey] == "inline label - visible with data") {
                if (currentFeature.get(currentKey) == null) {
                    continue;
                }
            }
            if (layer.get('fieldLabels')[currentKey] == "inline label - always visible" ||
                layer.get('fieldLabels')[currentKey] == "inline label - visible with data") {
                popupField += '<th>' + layer.get('fieldAliases')[currentKey] + '</th><td>';
            } else {
                popupField += '<td colspan="2">';
            }
            if (layer.get('fieldLabels')[currentKey] == "header label - visible with data") {
                if (currentFeature.get(currentKey) == null) {
                    continue;
                }
            }
            if (layer.get('fieldLabels')[currentKey] == "header label - always visible" ||
                layer.get('fieldLabels')[currentKey] == "header label - visible with data") {
                popupField += '<strong>' + layer.get('fieldAliases')[currentKey] + '</strong><br />';
            }
            if (layer.get('fieldImages')[currentKey] != "ExternalResource") {
				popupField += (currentFeature.get(currentKey) != null ? autolinker.link(currentFeature.get(currentKey).toLocaleString()) + '</td>' : '');
			} else {
				var fieldValue = currentFeature.get(currentKey);
        ...
        if (typeof clusteredFeatures !== "undefined") {
            if (doPopup) {
                for(var n=0; n<clusteredFeatures.length; n++) {
                    ...
                    const name = currentFeature.get('名称');
                    popupText += '<li><table>'
                    if (name) {
                        popupText += '<a>' + '<b>' + name + '</b>' + '</a>';
                    }
